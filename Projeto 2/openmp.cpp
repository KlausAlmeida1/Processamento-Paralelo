...
Versão revisada com rodadas múltiplas, média/desvio e saída em TXT por (N, threads).
*/
...
static void dgemm_omp(const double* A, const double* B, double* C, int N) {
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--runs" && i + 1 < argc) runs = stoi(argv[++i]);
            ...
            cout << "Uso: ./paralelo [--runs N] [--sizes 512,1024,...] "
                 << "[--threads 2,4,8,...] [--regen-per-run] [--out arquivo.txt] [--seed S]\n";
            return 0;
        }
    }
    ...
    ofs << "DGEMM OpenMP — Rodadas multiplas\n";
    ...
    for (int N : sizes) {
        ...
        for (int th : threads) {
            ...
            for (int r = 0; r < runs; ++r) {
                ...
                dgemm_omp(A.data(), B.data(), C.data(), N);
